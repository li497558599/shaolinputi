...
class QuestionController extends Controller {
    ...
    public function news_info($id) {
        ...
        $title = "查看详情";
        session("title", $title);
        $this->assign("info", M("question")->where("id = {$id}")->field("dis,content")->find());
        $this->display();
    }
    ...
    public function change_news($id) {
        ...
        $title = "修改家长问答";
        session("title", $title);
        $this->assign("info", M("question")->where("id = {$id}")->find());
        $this->assign("id", $id);
        $this->display();
    }
    ...
    public function update_news() {
        ...
        if ($info) {
            ...
            $data['pic'] = str_replace('./', '/', $data['pic']);
            $result = M("question")->where("id = {$id}")->field('pic')->find();
            $file = $result['pic'];
            $file = '.' . $file;
            $res = M("question")->where("id = {$id}")->save($data);
            if ($res) {
                unlink($file);
                die("<script>alert('恭喜你！修改成功！');location='" . __MODULE__ . "/Question/news';</script>");
            } else {
...
//            $data['description'] = $_POST['description']
    ...
    public function delete_news($id) {
        ...
        if ($res) {
            die("<script>alert('恭喜你！删除成功！');location='" . __MODULE__ . "/Question/news';</script>");
        } else {
...
//            $data['description'] = $_POST['description'];
            ...
            if ($res) {
                die("<script>alert('恭喜你！添加成功！');location='" . __MODULE__ . "/Question/news';</script>");
            } else {
